...
- **Workspace** — the overlay opened on a note: the note and the changes being prepared for it as **cards** on the left, a conversation with Claude on the right. A card holds **versions** (v0 = Anki, then Claude's proposals and the user's edits); a split adds **fragment** cards; cards can be **active** (the next message is about them), deleted or moved, and each carries a **flag toggle**: off — the default for the root — resolves the note at validation, on keeps it flagged with a comment written to `Back Extra` for a later pass. One « Valider » writes everything, all or nothing; the × discards everything. See [workspace.md](./workspace.md).
