...
## Note type across versions

In the [workspace](./workspace.md), a card's note type can change across [versions](./workspace.md#versions). Each version has a note type — it decides which fields the version has and is the type [written to Anki](./workspace.md#validation) when the user validates.

The original fields from Anki (**v0**) are always of the note's original type. A [proposal](./chat.md#proposal-tools) that names a different type sets a new note type for that version, and the version's fields are the complete set of the new type — not a merge with the previous version's fields. A version that does not name a type — a [hand edit](./workspace.md#editing), a proposal without the `model` parameter, or a proposal naming the same type — inherits the note type of the version it starts from. [Fragments](./workspace.md#split) inherit the note type of the parent card's shown version unless the proposal names another.
