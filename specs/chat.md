...
**Fields.** Field values in proposals are [raw](./notes.md#fields-are-raw-anki-html). On `propose_edit` they are the changed fields only, as complete raw values; the client merges them into the shown version. When `model` names a different [effective type](./notes.md#effective-type), `fields` are the complete set of the target type. On `propose_split`, `original` and each `new_notes` entry require `model` (the note type of the fragment); when it names a different type from the parent, `fields` are the complete set of that type (no merge with the parent's fields). The client infers the type from the fields as a fallback when Claude omits `model`. On `propose_create` every field is given.
