...
**Target.** A proposal names its target by workspace number (`"3"`) or Anki note id (a large number). A note id absent from the workspace adds the note as a card first, within the cap. An unknown id is an error result. A `propose_edit` on a deleted card puts the card back: a rewrite supersedes a deletion.
