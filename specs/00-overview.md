...
- **Source** — a document a deck was made from: an Obsidian note in the vault, or a PDF on disk (optionally restricted to a page range). A deck has a **corpus**: an ordered list of sources. A deck's effective corpus is its own sources plus each ancestor's, nearest first (`a::b::c`'s own, then `a::b`'s, then `a`'s). Every source has a stable **id**.
