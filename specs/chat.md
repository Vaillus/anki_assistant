...
Claude gets seven proposal tools. Each invocation is streamed to the client as a `proposal` event. Tools return `"ok"` to Claude immediately — accepting, editing or dropping the version is the user's decision, not Claude's.
...
| `propose_add_source` | `{ target, kind?, pages?, note?, anchor_note_ids?, rationale }` (deck = current deck; `kind` auto-detected from `target` when omitted, [sources.md § Source entry](./sources.md#source-entry)) | an inline proposal card in the log, applied on click via `POST /api/sources` |
...
`cards` is the workspace, root first; `original_fields` is present only when the shown version is not v0. `source_ids` is optional (default: no attached sources). `flagged_count` is optional (the deck's flagged-note count, for the prompt header). `messages` is the prior conversation as plain `{role, content: string}` turns — assistant text only; prior proposals and reads are not replayed. The client summarises them into the assistant text as bracket notations: « [proposition: edit → carte w1] », « [proposition: split → 3 cartes] », « [proposition: add_source (appliquée)] », « [proposition: create_source (appliquée)] », « [ajout: 6 notes] », « [version rejetée : w3 v2] », « [lecture: search_notes re:lagrang → 6 notes] », « [lecture: read_source → différentiabilité] », « [sources : [1] https://…, [2] https://…] » (the pages the reply cited, keeping the [n] markers in the text).
...
event: citation    data: {"n": 1, "url": "https://…", "title": "…", "cited_text": "…"}
event: added       data: {"id": "toolu_…", "note_ids": [1732375559262, …], "rationale": "…"}
event: proposal    data: {"id": "toolu_…", "kind": "edit", "input": {…}}            (+ "source_id" when kind is add_source or create_source)
...
Retrieval inside long PDFs (the user sets `pages` instead), persistence of conversations, Claude acting without a click, Claude editing note types (CSS, templates — read-only through `get_note_type`), creating or editing PDF sources, editing a web page (a web source is read-only; `propose_edit_source` refuses it).
