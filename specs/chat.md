...
**Fields.** Field values in proposals are [raw](./notes.md#fields-are-raw-anki-html). On `propose_edit` they are the changed fields only, as complete raw values; the client merges them into the shown version. When `model` names a different note type, `fields` are the complete set of the target type. On `propose_split`, `original` and each `new_notes` entry accept an optional `model`; when it names a different type, `fields` are the complete set of that type (no merge with the parent's fields). On `propose_create` every field is given.
